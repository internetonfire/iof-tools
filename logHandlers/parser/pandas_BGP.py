...
if __name__ == '__main__':
    ...
    data_set, time_data = hf.fill_run_table(hf.index_names, indexes[0], 
            indexes[1], indexes[2],
            indexes[3], mode='INCREASING_L', samples=hf.samples)
    run_table_index = pd.MultiIndex.from_product(indexes, names=hf.index_names)
    run_table = pd.DataFrame(data_set, index=run_table_index, columns=hf.column_names)
    update_table_index = pd.timedelta_range(0, periods=hf.samples, freq=hf.delta)
    update_table = pd.DataFrame(time_data, index=update_table_index, 
                                columns=run_table_index)
    args = hf.parse_args()
    hf.check_data(update_table, run_table)
    T_ASes = set()
    if args.G:
    ...
    ct = hf.conv_time_per_distance(run_table, update_table.index)
    pl = ct.plot(title="Convergence time by distance from T_r")
    pl.set_xlabel('time')
    pl.set_ylabel('# of ASes')
    ct = hf.conv_time_per_distance(run_table, update_table.index, column='distance_AS_after_t')
    pl = ct.plot(title="Convergence time by hops after T nodes")
    pl.set_xlabel('time')
    pl.set_ylabel('# of ASes')
    ct = hf.conv_time_per_distance(run_table, update_table.index, column='distance_AS_before_t')
